...
class OrderStatusView(QWidget):
    def __init__(self, db_connection):
        ...
        self.setWindowTitle("Управление статусами заказов")

        self.layout = QVBoxLayout(self)

        info_label = QLabel("Управление статусами заказов инвентаризации.")
        self.layout.addWidget(info_label)

        # --- Настройки модели и таблицы ---
        self.table_name = "Order_status" # Название таблицы
        self.column_names = [col.split()[0] for col in DATABASE_SCHEMA[self.table_name] if not col.strip().startswith("FOREIGN KEY")]
        self.unique_column = "order_status" # Столбец для проверки уникальности
        ...
        header_labels = ["ID", "Статус заказа"]
        ...
        self.order_status_input.setPlaceholderText("Введите новый статус заказа")
        add_layout.addWidget(self.order_status_input)

        add_button = QPushButton("Добавить статус")
    ...
    def _add_item(self):
        ...
        item_name = self.order_status_input.text().strip()

        if not item_name:
            QMessageBox.warning(self, "Предупреждение", "Пожалуйста, введите название статуса заказа.")
            return

        query = QSqlQuery(self.db)
        query.prepare(f"SELECT COUNT(*) FROM {self.table_name} WHERE order_status = ?")
        query.addBindValue(item_name)
        if query.exec_() and query.next():
            if query.value(0) > 0:
                QMessageBox.warning(self, "Предупреждение", f"Статус заказа '{item_name}' уже существует.")
                return

        row_count = self.model.rowCount()
        self.model.insertRow(row_count)
        # Столбец 1 - 'order_status'
        self.model.setData(self.model.index(row_count, 1), item_name)

        if self.model.submitAll():
            print(f"Статус заказа '{item_name}' успешно добавлен.")
            self.order_status_input.clear()
        else:
            print("Ошибка при добавлении статуса заказа:", self.model.lastError().text())
            QMessageBox.critical(self, "Ошибка", f"Не удалось добавить статус заказа: {self.model.lastError().text()}")
            self.model.revertAll()

    def _delete_selected_item(self):
        """Удаляет выбранный статус заказа."""
        selected_indexes = self.table_view.selectedIndexes()
        if not selected_indexes:
            QMessageBox.warning(self, "Предупреждение", "Пожалуйста, выберите статус заказа для удаления.")
            return

        selected_index = selected_indexes[0]
        row = selected_index.row()

        item_name = self.model.data(self.model.index(row, 1), Qt.DisplayRole) # Столбец 1 - order_status

        reply = QMessageBox.question(self, "Подтверждение удаления",
                                     f"Вы уверены, что хотите удалить статус заказа '{item_name}'?",
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.No)

        if reply == QMessageBox.Yes:
            if self.model.removeRow(row):
                if self.model.submitAll():
                    print(f"Статус заказа '{item_name}' успешно удален.")
                else:
                    print("Ошибка при сохранении удаления:", self.model.lastError().text())
                    QMessageBox.critical(self, "Ошибка", f"Не удалось удалить статус заказа: {self.model.lastError().text()}")
                    self.model.revertAll()
            else:
    ...
    def _init_new_row(self, row, record):
        ...
        pass # Для Order_status нет специфичных значений по умолчанию
